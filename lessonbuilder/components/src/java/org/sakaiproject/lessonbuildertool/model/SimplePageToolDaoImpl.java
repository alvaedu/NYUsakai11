...
public class SimplePageToolDaoImpl extends HibernateDaoSupport implements SimplePageToolDao {
	...
	public String getPageUrl(long pageId) {
		...
		if (sitePage == null) {
			log.error("Failed to find Sakai SitePage for lessons page with id: {}", pageId);
		}
		...
		if (tools.size() == 0) {
			log.error("Failed to find a lessonbuilder tool for for lessons page with id: {}", pageId);
		}
